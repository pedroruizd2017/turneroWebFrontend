...
//                }
        ...
        .controller('cambioCtrl', ['$state', '$compile', '$scope', 'config', '$cookies', '$interval', 'RetiroPedidoService', function ($state, $compile, $scope, config, $cookies, $interval, RetiroPedidoService) {
//                setTimeout(alert("HOLA HORACIO"), 2000);
                $scope.retiroPedido = [];
                $scope.cargarDatos = function () {
                    RetiroPedidoService.listar().success(function (d) {
...
//                            var newstr = descriRol.replace(" ", "_");
                ...
                $scope.dblClick = function (id) {
                    var xyz = 0;
                    var descri = "";
                    var num = "";
//                    alert("HOLA")
                    RetiroPedidoService.listarPorId(id).success(function (d) {
//                        alert(d.facturaClienteCab.idFacturaClienteCab)
                        num = d.numero;
                        RetiroPedidoService.listarDetalleFactura(d.facturaClienteCab.idFacturaClienteCab).success(function (data) {
//                            alert(data.length)
                            angular.forEach(data, function (value, key) {
                                descri += (xyz + 1) + ") " + value.descripcion.toUpperCase() + " ";
                                xyz++;
                                if (data.length === xyz) {
...
//                            $scope.listaBoxSinSelecionar();
                ...
                $scope.listaBox = function () {
                    ...
                    RetiroPedidoService.listarFullListo().success(function (d) {
                        ...
                        var datoForm = "";
                        $("#listSeleccionado").html("");
                        angular.forEach(d, function (value, key) {
                            $scope.listaDeSeleccionados += "<li class=\"list-group-item ng-binding\" ng-dblclick='dblClick(" + value.idRetiro + ")'><b style=\"color: red; font-size: 22px; line-height: 1.4em;\">" + value.numero + " </b><b style=\"font-size: 20px;\"> - " + value.hora +
                                    "</b><input type=\"hidden\" id=\"dato" + value.numero.replace(" ", "_") + "\" value=\"" + value.idRetiro
                                    + "\"></li>";
                            datoForm += "<li class=\"list-group-item ng-binding\" ng-dblclick='dblClick(" + value.idRetiro + ")'><b style=\"color: red; font-size: 22px; line-height: 1.4em;\">" + value.numero + " </b><b style=\"font-size: 20px;\"> - " + value.hora +
                                    "</b><input type=\"hidden\" id=\"dato" + value.numero.replace(" ", "_") + "\" value=\"" + value.idRetiro
                                    + "\"></li>";
                            dato = true;
                        });
                        var $el = $(datoForm).appendTo('#listSeleccionado');
                        $compile($el)($scope);
...
//                            actives.clone().appendTo('.list-left ul');
        ...
        .controller('cambioDatoCtrl', ['$state', '$compile', '$scope', 'config', '$cookies', '$interval', 'RetiroPedidoService', function ($state, $compile, $scope, config, $cookies, $interval, RetiroPedidoService) {
//                setTimeout(alert("HOLA HORACIO"), 2000);
                $scope.retiroPedido = [];
                $scope.listoPorRetirar = [];
                $scope.dataProducto = [];
                $scope.cargarDatos = function () {
                    RetiroPedidoService.listar().success(function (d) {
                        angular.forEach(d, function (value, key) {
...
//                        alert(d.facturaClienteCab.idFacturaClienteCab)
                        ...
                        RetiroPedidoService.listarDetalleFactura(d.facturaClienteCab.idFacturaClienteCab).success(function (data) {
//                            alert(data.length)
                            console.log("Observación: " + decode_utf8(d.facturaClienteCab.observacion));
                            if (decode_utf8(d.facturaClienteCab.observacion) === "null") {
                                $("#observacion").fadeOut("fast");
                            } else {
                                $scope.detalleObs = decode_utf8(d.facturaClienteCab.observacion);
                                $("#observacion").fadeIn("fast");
                            }
                            angular.forEach(data, function (value, key) {
                                $scope.data = {};
                                ...
                                descri += (xyz + 1) + ") " + value.descripcion.toUpperCase() + " ";
                                $scope.data.id = xyz;
                                $scope.data.canti = value.cantidad;
                                $scope.data.descri = value.descripcion.toUpperCase();
                                $scope.dataProducto.push($scope.data);
                                xyz++;
                                if (data.length === xyz) {
                                    descri = descri.substring(0, (descri.length - 1));
//                                    alert(descri)
...
//                    $scope.retiroPedido = [];
                    var valorSeleccionado = "";
                    $("#lstBox1 option").each(function (index) {
                        valorSeleccionado = $.trim($(this).text());
//                        alert(valorSeleccionado)
                        valorSeleccionado = valorSeleccionado.trim();
//                        alert(valorSeleccionado)
                        var arr = valorSeleccionado.split('-');
                        if ($("#dato" + arr[0]).val() === undefined) {
                            var nume = $.trim(arr[0]);
                            console.log(nume + "GS")
                            RetiroPedidoService.buscarPorNumeroPedido(nume).success(function (data) {
                                RetiroPedidoService.actualizarPreparado(data.idRetiro).success(function (d) {
                                    if (d === true) {
                                        console.log("DATOS ACTUALIZADOS CORRECTAMENTE");
//                                        $scope.listaBoxSinSelecionar();
                                    } else {
...
//                    alert($('#lstBox2').val());
                    $("#lstBox2 option").each(function (index) {
                        ...
                        valorSeleccionado = valorSeleccionado.trim();
                        var arr = valorSeleccionado.split('-');
                        var nume = arr[0].replace(" ", "");
//                        alert($('select').val() + " - " + $("#dato" + arr[0]).val() + " - " + nume + "-")
                        RetiroPedidoService.buscarPorNumeroPedido(nume).success(function (data) {
                            if (parseInt(data.idRetiro) === parseInt($('#lstBox2').val())) {
                                RetiroPedidoService.actualizarEntregado(data.idRetiro).success(function (d) {
                                    if (d === true) {
                                        console.log("DATOS ACTUALIZADOS CORRECTAMENTE");
                                    } else {
                                        console.log("DATOS SIN ACTUALIZARSE");
...
//                        alert(valorSeleccionado)
                        ...
                        if ($("#dato" + arr[0]).val() === undefined) {
                            var nume = $.trim(arr[0]);
                            RetiroPedidoService.buscarPorNumeroPedido(nume).success(function (data) {
                                RetiroPedidoService.actualizarListo(data.idRetiro).success(function (d) {
                                    if (d === true) {
                                        console.log("DATOS ACTUALIZADOS CORRECTAMENTE");
//                                $scope.listaBox();
                                        console.log("DATOS ACTUALIZADOS");
...
//                            $scope.listaBoxSinSelecionar();
                ...
                $scope.cambiarDerecha = function () {
//                    $('select').moveToListAndDelete('#lstBox1', '#lstBox2');
//                    sourceList, destinationList
                    var opts = $('#lstBox1' + ' option:selected');
                    if (opts.length === 0) {
                        $("body").overhang({
                            type: "error",
                            message: "NADA POR MOVER.",
                            duration: 1
//                            overlay: true
                        });
                    } else {
                        $(opts).remove();
                        $('#lstBox2').append($(opts).clone());
                        $scope.front();
                        e.preventDefault();
                    }
                }
                $scope.cambiarIzquierda = function () {
...
//                        $scope.back();
                ...
                $scope.listaBox = function () {
                    var dato = false;
                    $scope.listoPorRetirar = [];
                    $('#lstBox1').find('option').remove();
                    $('#lstBox2').find('option').remove();
                    RetiroPedidoService.listarFullListo().success(function (d) {
//                        $scope.listaDeSeleccionados = "";
//                        var datoForm = "";
//                        $("#listSeleccionado").html("");
                        $scope.listoPorRetirar = d;
//                        angular.forEach(d, function (value, key) {
//                            $scope.listaDeSeleccionados += "<li class=\"list-group-item ng-binding\" ng-dblclick='dblClick(" + value.idRetiro + ")'><b style=\"color: red; font-size: 22px; line-height: 1.4em;\">" + value.numero + " </b><b style=\"font-size: 20px;\"> - " + value.hora +
//                                    "</b><input type=\"hidden\" id=\"dato" + value.numero.replace(" ", "_") + "\" value=\"" + value.idRetiro
//                                    + "\"></li>";
...
//                            actives.clone().appendTo('.list-left ul');
        ...
        .controller('clienteCtrl', ['$state', '$scope', '$base64', 'config', '$cookies', '$interval', 'RetiroPedidoService', 'ClienteService', 'ClientePendienteService', 'PublicidadService', 'ModoVentaService', function ($state, $scope, $base64, config, $cookies, $interval, RetiroPedidoService, ClienteService, ClientePendienteService, PublicidadService, ModoVentaService) {
//                setTimeout(alert("HOLA HORACIO"), 2000);
                $scope.retiroPedido = [];
                $scope.cargarDatos = function () {
                    RetiroPedidoService.listar().success(function (d) {
...
//                            var newstr = descriRol.replace(" ", "_");
                ...
                $scope.verificarLoginInicial = function () {
                    ModoVentaService.sinLogin().success(function (d) {
                        if (d === true) {
                            $("#ciCliente").val("123");
                            $scope.ingresar();
                        }
...
//                            console.log("LISTO -->> " + valorSeleccionado + " ID: " + $("#dato" + arr[0]).val());
                ...
                $scope.atrasLogin = function () {
                    ...
                    $("#nomCliente").css("display", "none");
                    $("#apeCliente").css("display", "none");
...
//                                    });
                                    ...
                                    $("#nomCliente").css("display", "block");
                                    $("#apeCliente").css("display", "block");
...
//                        $scope.listaBox();
                        ...
                        $("body").overhang({
                            type: "error",
                            message: "AUN NO SE HA ESTABLECIDO CONEXION CON EL SERVIDOR.",
                            duration: 1,
                            overlay: true
                        });
                    });
...
//                        console.log("PREPARACION -->> " + valorSeleccionado + " ID: " + $("#dato" + arr[0]).val());
        ...
        .controller('mainMenuCtrl', ['$state', '$filter', '$base64', '$compile', '$scope', 'config', '$cookies', '$interval', 'RetiroPedidoService', 'ClienteService', 'EmpresaService', 'SeccionService', 'ArticuloService', 'ServPendienteService', 'ClientePendienteService', 'SupervisorService', 'SucursalService', 'TimbradoService', 'CajaService', 'TalonarioSucursalService', 'RangoRetiroPedidoService', 'PublicidadService', 'ModoVentaService', function ($state, $filter, $base64, $compile, $scope, config, $cookies, $interval, RetiroPedidoService, ClienteService, EmpresaService, SeccionService, ArticuloService, ServPendienteService, ClientePendienteService, SupervisorService, SucursalService, TimbradoService, CajaService, TalonarioSucursalService, RangoRetiroPedidoService, PublicidadService, ModoVentaService) {
                $scope.srvPendiente = [];
                $scope.formaPago = [];
...
//                    alert($scope.xxx.facturaClienteCab.fechaEmision + " - " + $scope.xxx.facturaClienteCab.nroFactura + " - " + $scope.xxx.facturaClienteCab.montoFactura)
                    RangoRetiroPedidoService.actualizarObtenerRangoObjectActual(config.sucursal).success(function (datito) {
                        ...
                        ClienteService.enviarPrueba($scope.xxx.facturaClienteCab, config.caja, nroPedido, $("#txtObservacion").val()).success(function (da) {
...
//                                                                            alert($("#facturaImpresion").html());
                                                                                ModoVentaService.sinLogin().success(function (opq) {
                                                                                    if (opq === true) {
                                                                                        location.href = "./menu.html";
                                                                                    } else {
...
//                    alert("ID ARTICULO: " + id + " - " + articulo);
                    ...
                    if ($scope.srvPendiente.length === 0) {
                        ArticuloService.listarPorCodigo(id).success(function (d) {
                            ...
                            var porc = 0;
                            var cantidad = 1;
                            if (parseInt(d.stockActual) >= 1) {
                                var dif = parseInt(d.stockActual) - 1;
                                ArticuloService.actualizarCantidad(d.idArticulo, dif).success(function (xau) {
                                    if (xau === true) {
...
//                                        $scope.cargarCarrito(true);
                                ...
                                $("body").overhang({
                                    type: "error",
                                    message: "EL ARTICULO YA NO DISPONE DE STOCK PARA SU VENTA.",
                                    duration: 1
//                                    overlay: true
                                });
                            }
...
//                            alert(parseInt(data.codArticulo) + " - " + parseInt(id))
                            if (parseInt(data.codArticulo) === parseInt(id)) {
                                estado = true;
                                var canti = parseInt(data.cantidad) + 1;

                                ArticuloService.listarPorCodigo(id).success(function (acaNomas) {
                                    if (parseInt(acaNomas.stockActual) >= 1) {
                                        var dif = parseInt(acaNomas.stockActual) - 1;
//                                        alert("act ->" + acaNomas.idArticulo + " - " + dif + " - " );
                                        ArticuloService.actualizarCantidad(acaNomas.idArticulo, dif).success(function (xau) {
                                            if (xau === true) {
                        ...
                        ServPendienteService.buscandoServicios(idCP, 0).success(function (d) {
                            ...
                            angular.forEach(d, function (data, key) {
                                ...
                                if (data.idServPendiente === idServ) {
                                    ...
                                    ArticuloService.listarPorCodigo(data.codArticulo).success(function (acaNomas) {
//                                        if (parseInt(acaNomas.stockActual) >= 1) {
                                        var dif = parseInt(acaNomas.stockActual) + 1;
                                        ArticuloService.actualizarCantidad(acaNomas.idArticulo, dif).success(function (xau) {
                                            if (xau === true) {
                                                ...
                                                if (data.cantidad === 0) {
                                                    ...
                                                    $scope.srvPendiente = $filter('orderBy')($scope.srvPendiente, 'descripcion', false)
                                                }
